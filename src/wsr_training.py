...
from world_of_supply_rllib_models.facility_net_model import FacilityNet
#import
...
env = WorldOfSupplyEnv(env_config)
...
def play_baseline(n_iterations):
  ...
  HandCodedTrainer = tt.build_trainer('HandCoded', SimplePolicy)
  ext_conf = {
      'multiagent': {
        'policies': filter_keys(policies, ['baseline_producer', 'baseline_consumer']),
        'policy_mapping_fn': lambda agent_id: 'baseline_producer' if utils_is_producer_agent(agent_id) else 'baseline_consumer',
